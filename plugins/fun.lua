...
local function calc(exp)
	...
	text = nil
	if c == 200 then
    text = 'Result = '..b..'\n🆔 Channel ™ : '..check_markdown('@Shield_Tm')..'\n=======================\n🇸 🇭 🇮 🇪 🇱 🇩™'
	elseif c == 400 then
		text = b
	else
			...
			function tophoto_cb(arg,data)
				if data.content_.sticker_ then
					...
					if file_exi(tostring(name), tostring(apath), tostring(pasvand)) then
						...
						tdcli.sendPhoto(msg.to.id, 0, 0, 1, nil, pfile, "🆔 Channel ™ : @Shield_Tm", dl_cb, nil)
					else
			...
			function tosticker_cb(arg,data)
				if data.content_.ID == 'MessagePhoto' then
					...
					if file_exi(file..'_(1).jpg', tcpath..'/data/photo', 'jpg') then
						...
						tdcli.sendDocument(msg.chat_id_, 0, 0, 1, nil, pfile, '🆔 Channel ™ : @Shield_Tm', dl_cb, nil)
					else
...
if matches[1] == 'voice' then
 local text = matches[2]
  ...
  local file = download_to_file(url,'SDP_Team.mp3')
 				tdcli.sendDocument(msg.to.id, 0, 0, 1, nil, file, '🆔 Channel ™ : @Shield_Tm', dl_cb, nil)
   end
end
...
--------------------------------
	if matches[1]:lower() == "photo" then 
		...
		tdcli.sendPhoto(msg.to.id, 0, 0, 1, nil, file, "🆔 Channel ™ : @Shield_Tm", dl_cb, nil)
	end
